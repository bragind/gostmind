    ...
    async def execute(self, query: Query) -> QueryResult:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                gost_number = metadatas[i].get("gost_number", "Неизвестный ГОСТ") if i < len(metadatas) else "Неизвестный ГОСТ"
...
Если в контексте нет ответа, скажи об этом."""
            ...
            confidence = max(0.0, min(1.0, 1.0 - avg_distance))  # Преобразуем расстояние в уверенность
